...
document.addEventListener("DOMContentLoaded", function () {
  let url = "http://localhost/compareit/webapp/php/api.php";
  ...
  function changeTopDashboardData(viewData) {
    ...
    for (let id of ids) {
      document.getElementById(id + "-count").textContent =
        viewData[id]["count"];
      document.getElementById(id + "-growth").textContent =
        viewData[id]["growth"].toFixed(0) + "% in the last 30 days";
  ...
  function changeDashboardGraphData(viewData) {
    updateChart(
      pricesChart,
      viewData["prices"]["labels"],
      viewData["prices"]["data_points"],
      "Average Price"
    );
    updateChart(
      wishlistsChart,
      viewData["wishlists"]["labels"],
      viewData["wishlists"]["data_points"],
      "Average Wishlists Saves"
    );
  }
